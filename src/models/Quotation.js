...
module.exports = mongoose.model("Quotation", new mongoose.Schema({
  ...
  clientName: { type: String, required: true }, clientAddress: { type: String, default: "" },
  issueDate: { type: Date, default: Date.now },
  validUntil: { type: Date, required: true }, subject: { type: String, required: true, trim: true },
  items: [{ description: { type: String, required: true, trim: true }, unit: { type: String, default: "Nos" }, quantity: { type: Number, required: true, min: 0.01 }, rate: { type: Number, required: true, min: 0 }, amount: { type: Number, required: true, min: 0 }, remark: { type: String, default: "", trim: true } }],
