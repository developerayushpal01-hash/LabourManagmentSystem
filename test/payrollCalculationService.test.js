...
test("overtime from hours and rate", () => assert.equal(base({ attendanceRecords: [{ status:"PRESENT",overtimeHours:2,overtimeRate:120 }] }).overtime, 240));
test("overtime uses daily wage on an 8-hour basis for decimal hours", () => assert.equal(base({ labour: { dailyWage: 800, isPFApplicable: true, pfUanNumber: "100000000001", isESICApplicable: true, esicIpNumber: "1000000001" }, skill: { defaultDailyWage: 600 }, attendanceRecords: [{ status:"PRESENT",overtimeHours:2.5 }], payrollSetting: { basicPercentage: 40, hraPercentage: 20, allowanceCalculationMode: "REMAINING", isPFEnabled: false, isESICEnabled: false, roundOffSalary: false } }).overtime, 250));
